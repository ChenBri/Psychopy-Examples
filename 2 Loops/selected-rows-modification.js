/*********************************** 
 * Selected-Rows-Modification Test *
 ***********************************/
...
let expName = 'selected-rows-modification';  // from the Builder filename that created this script
let expInfo = {
...
flowScheduler.add(CreateRoutineBegin());
flowScheduler.add(CreateRoutineEachFrame());
flowScheduler.add(CreateRoutineEnd());
const loop_2LoopScheduler = new Scheduler(psychoJS);
flowScheduler.add(loop_2LoopBegin(loop_2LoopScheduler));
flowScheduler.add(loop_2LoopScheduler);
flowScheduler.add(loop_2LoopEnd);
...
psychoJS.start({
  ...
  expInfo: expInfo,
  resources: [
    {'name': '../saraalkhayarin/Flow.png', 'path': '../saraalkhayarin/Flow.png'},
...
async function experimentInit() {
  // Initialize components for Routine "Create"
  CreateClock = new util.Clock();
  ...
  // Initialize components for Routine "ad"
  adClock = new util.Clock();
...
function CreateRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'Create' ---
    t = 0;
    CreateClock.reset(); // clock
    frameN = -1;
    ...
    for (const thisComponent of CreateComponents)
...
function CreateRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'Create' ---
    // get current time
    t = CreateClock.getTime();
...
function CreateRoutineEnd(snapshot) {
  return async function () {
    //--- Ending Routine 'Create' ---
    for (const thisComponent of CreateComponents) {
      if (typeof thisComponent.setAutoDraw === 'function') {
        thisComponent.setAutoDraw(false);
      }
    }
    // the Routine "Create" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset();
...
function loop_2LoopBegin(loop_2LoopScheduler, snapshot) {
  return async function() {
    ...
    loop_2 = new TrialHandler({
      ...
      trialList: TrialHandler.importConditions(psychoJS.serverManager, 'loopTemplate2.xlsx', ((loop2Reps.toString() + ":") + (loop2Reps + 5).toString())),
    ...
    for (const thisLoop_2 of loop_2) {
      ...
      loop_2LoopScheduler.add(loop_1LoopBegin(loop_1LoopScheduler, snapshot));
      loop_2LoopScheduler.add(loop_1LoopScheduler);
      loop_2LoopScheduler.add(loop_1LoopEnd);
      loop_2LoopScheduler.add(adRoutineBegin(snapshot));
      loop_2LoopScheduler.add(adRoutineEachFrame());
      loop_2LoopScheduler.add(adRoutineEnd(snapshot));
...
function loop_1LoopBegin(loop_1LoopScheduler, snapshot) {
  return async function() {
    ...
    loop_1 = new TrialHandler({
      ...
      extraInfo: expInfo, originPath: undefined,
      trialList: TrialHandler.importConditions(psychoJS.serverManager, 'loopTemplate1.xlsx', ((loop1Reps.toString() + ":") + (loop1Reps + 9).toString())),
    ...
    for (const thisLoop_1 of loop_1) {
      ...
      loop_1LoopScheduler.add(trial_1RoutineBegin(snapshot));
      loop_1LoopScheduler.add(trial_1RoutineEachFrame());
      loop_1LoopScheduler.add(trial_1RoutineEnd(snapshot));
...
function trial_1RoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    text.setText(("Stim 1: " + stim_1.toString()));
...
var adComponents;
function adRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'ad' ---
    t = 0;
    adClock.reset(); // clock
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    ...
    // update component parameters for each repeat
    image_2.setPos([stim_1, stim_1]);
    image_2.setImage('C:/Users/chenb/Desktop/saraalkhayarin/Flow.png');
    // keep track of which components have finished
    adComponents = [];
    adComponents.push(image_2);
    
    for (const thisComponent of adComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
var frameRemains;
function adRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'ad' ---
    // get current time
    t = adClock.getTime();
    frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
    // update/draw components on each frame
...
function adRoutineEnd(snapshot) {
  return async function () {
    //--- Ending Routine 'ad' ---
    for (const thisComponent of adComponents) {
      if (typeof thisComponent.setAutoDraw === 'function') {
...
function trial_2RoutineBegin(snapshot) {
  return async function () {
    ...
    continueRoutine = true; // until we're told otherwise
    // update component parameters for each repeat
    text_2.setText(("Stim 2: " + stim_2.toString()));
    ...
    trial_2Components.push(mouse_2);
